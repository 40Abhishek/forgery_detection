...
def check_metadata(reader):
    """
    What it does:
        Reads the PDF metadata dictionary — every PDF has one.
        It contains fields like who created it, with what software,
        and when it was created vs last modified.
    ...
    Why this matters:
        A genuine university marksheet is generated by the
        university's system — producer will be something like
        "Microsoft Word" or "ReportLab". If we see "iLovePDF"
        or "Sejda" as the producer, someone edited it online.
    """
    ...
    if not info:
        flags.append("No metadata found — may have been stripped during editing")
        return {
            "metadata"  : metadata,
            "flags"     : flags,
            "suspicious": True,
    ...
    metadata = {
        ...
        "created" : created  or "Not specified",
        "modified": modified or "Not specified"
    }
    ...
    if created and modified and created != modified:
        try:
            ...
            dt_created  = parse_pdf_date(created)
            dt_modified = parse_pdf_date(modified)
            gap_days    = (dt_modified - dt_created).days

            if gap_days > 30:
                flags.append(
                    f"Large gap between creation and modification: {gap_days} days"
                )
            elif gap_days < 0:
...
def run_pdf_forensics(pdf_path):
    """
    Call this from your pipeline for vector PDFs only.
    Stage 1 routes vector PDFs directly here, bypassing Stage 2 and 3.

    Args:
        pdf_path : path to the vector PDF file
    ...
    print("-" * 50)
    print(f"  PDF Score  : {pdf_score} / 100")
...
if __name__ == "__main__":
    import sys
    path   = sys.argv[1] if len(sys.argv) > 1 else "test_document.pdf"
    result = run_pdf_forensics(path)
    print(f"\n  → PDF score for Risk Engine : {result['pdf_score']}")